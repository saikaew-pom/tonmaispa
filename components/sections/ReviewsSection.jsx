    ...
    <section id="reviews" style={{ padding: 'clamp(64px,10vw,128px) clamp(18px,4vw,40px)', background: '#E8EDE9' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div data-reveal style={{ opacity: 0, transform: 'translateY(24px)', transition: 'opacity .8s ease, transform .8s ease', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 'clamp(28px,4vw,56px)', alignItems: 'center', marginBottom: 'clamp(40px,5vw,56px)' }}>
          <div>
            <div style={{ font: '600 11px Inter,sans-serif', letterSpacing: 3, textTransform: 'uppercase', color: '#C4924A' }}>{t(dict, 'home.reviews.eyebrow')}</div>
          ...
          <div style={{ display: 'flex', gap: 'clamp(20px,3vw,40px)', alignItems: 'center', flexWrap: 'wrap' }}>
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ font: '600 10px Inter,sans-serif', letterSpacing: 2, textTransform: 'uppercase', color: '#9B9390', marginTop: 6 }}>{t(dict, 'home.reviews.googleRating')}</div>
            </div>
            ...
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ font: '600 10px Inter,sans-serif', letterSpacing: 2, textTransform: 'uppercase', color: '#9B9390', marginTop: 10 }}>{t(dict, 'home.reviews.verifiedReviews')}</div>
            </div>
          </div>
        </div>
