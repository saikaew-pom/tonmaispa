...
import { confirmRescheduleDraft } from '@/lib/chat-booking'
...
export async function POST(req) {
  ...
  if (!parsed.success) return Response.json({ error: 'Invalid reschedule confirmation.' }, { status: 400 })
  ...
  if (!fullBookingEnabled) {
    return Response.json({ error: 'Chat booking changes are not currently available.' }, { status: 503 })
  }

  const result = await confirmRescheduleDraft(admin, { ...parsed.data, settings })
