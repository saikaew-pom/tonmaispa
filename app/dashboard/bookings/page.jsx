...
async function getData() {
  ...
  const [bookingsRes, treatmentsRes, therapistsRes, settingsRes] = await Promise.all([
    admin.from('bookings')
      .select('id, ref_code, guest_name, guest_phone, guest_email, customer_id, treatment_id, therapist_id, date, time_slot, duration, price, status, source, notes, staff_notes, last_email_sent_at, last_email_status, last_whatsapp_sent_at, last_whatsapp_status, whatsapp_reminder_sent_at, whatsapp_reminder_status, whatsapp_followup_sent_at, whatsapp_followup_status, spa_treatments(name)')
