...
// The GA4 tracking script loads on every page, dashboard included — so
// without this filter, staff logging into /dashboard every day would show
// up as "visitors" and skew every metric. This report is about the public
// site's real visitors, so admin-only paths are excluded from all queries.
const ADMIN_PATH_PREFIXES = ['/dashboard', '/login', '/forgot-password', '/reset-password']
const EXCLUDE_ADMIN_FILTER = {
  notExpression: {
    orGroup: {
      expressions: ADMIN_PATH_PREFIXES.map(prefix => ({
