...
export function buildSystemPrompt({ treatments, settings, bookingEngineEnabled }) {
  ...
  const bookingInstruction = bookingEngineEnabled
    ? `When a guest wants to book, use the check_availability tool (pass the treatment's [id] and the duration in minutes) to find open slots, then use create_booking to confirm. Walk them through date → time → their name and phone. Availability is per-treatment — always check with the specific treatment the guest wants. Never announce a time as available without a tool result from this conversation confirming it.`
