...
- To submit the booking request → use create_booking. It re-validates capacity; if the slot just filled, it returns the nearest open times to offer instead.
...
- A create_booking result creates a pending booking request. Say it is received with its reference; do not call it confirmed unless the tool explicitly returns confirmed status.
...
export const CHATBOT_TOOLS = [
  ...
  {
    name: 'create_booking',
    description: 'Create a pending booking request. Only use when booking engine is enabled AND the guest has provided all required details. The system re-validates capacity — if the slot filled up, you get an error with the nearest open times to offer instead. Never describe the result as confirmed unless the result explicitly says confirmed.',
    input_schema: {
      type: 'object',
      properties: {
        guest_name:     { type: 'string' },
