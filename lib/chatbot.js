...
- Once date, time, treatment, duration and the guest's name are complete → use prepare_booking. It re-validates capacity and creates a review summary, but it does NOT make a booking.
...
- Never write or invent a booking review summary yourself. A real review summary and Confirm booking button appear only after a successful prepare_booking result.
- The review card itself asks the guest to enter their country, phone number and email directly — you do not need to collect phone or email yourself in chat, and you should never guess or format a phone number for them. If the guest already mentioned a phone number in chat, that's fine as a placeholder, but the guest confirms the final phone (with country code) and email themselves on the review card before the Confirm button unlocks.
- Tell the guest to review the summary, fill in their country/phone/email on that card, and press the Confirm booking button. Only that button can submit the request. Never claim it was submitted merely because prepare_booking succeeded.
...
Always walk them through date → time → their name. Never announce a time or treatment as available without a tool result from THIS conversation — don't assume, and never say a checked slot is held.`
...
export const CHATBOT_TOOLS = [
  ...
  },
  {
    ...
    description: 'Prepare the server-backed booking review card. You MUST use this when the guest asks to prepare, review, or show a booking summary and the treatment/date/time/duration/name are complete. Phone and email are NOT required here — the review card itself collects and validates those directly from the guest right before they confirm. This tool is safe to call when the guest says not to submit: it never inserts or holds a booking. The guest must press the separate Confirm booking button before the request is inserted. Never make a review summary in text without this tool.',
    input_schema: {
      type: 'object',
      properties: {
        guest_name:     { type: 'string' },
      ...
      required: ['guest_name', 'date', 'time_slot', 'duration'],
    },
  },
]
