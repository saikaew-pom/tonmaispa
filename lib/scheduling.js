...
// Which kind of room a treatment needs: 'any' (any free room — most things),
// 'wet' (needs the shower — body scrubs), 'couples' (the couple's room), or
// 'none' (no room at all — reflexology is done in chairs, thermal passes are
// facility access). The DB trigger is the authority and stamps the same value
// onto bookings.room_requirement; this is the JS pre-check's view of it.
// Falls back to 'any' — the conservative answer, since 'any' is still bounded
// by the total-rooms check. (migration 033)
export async function getRoomRequirement(admin, treatmentId) {
  if (!treatmentId) return 'any'
  // select('*') so this can't 400 before the migration adds the column.
  const { data } = await admin.from('spa_treatments').select('*').eq('id', treatmentId).maybeSingle()
  const req = data?.room_requirement
  return ['any', 'wet', 'couples', 'none'].includes(req) ? req : 'any'
}

// How many of each SPECIALISED room exist ({ wet: 1, couples: 1 }). Standard
// rooms deliberately have no entry: a standard treatment can use any free
// room, so it's already bounded by room_capacity's total.
export async function getRoomTypeCapacity(admin) {
  const { data, error } = await admin.from('room_type_capacity').select('room_type, room_count')
  if (error) return {} // pre-migration: no typed limits, stay lenient
  return Object.fromEntries((data ?? []).map(r => [r.room_type, r.room_count]))
}
...
// `roomRequirement` is what the PROSPECTIVE booking needs (see
// getRoomRequirement). Mirrors the trigger in migration 033:
//   'none'            → needs no room at all; never blocked, never counted
//   'any'             → bounded by the total room count
//   'wet' | 'couples' → bounded by the total AND by that type's own count
// Rooms are deliberately not hard-partitioned, so a plain massage may occupy
// the couple's room; only the scarce specialised types are rationed.
export async function getAvailableRoomCount(admin, { date, startTime, endTime, excludeBookingId, maxConcurrent, bufferMin = 0, roomRequirement = 'any' }) {
  // Reflexology (chairs) and thermal passes tie up no room — they must never
  // consume capacity, and can never be refused for lack of one.
  if (roomRequirement === 'none') return Number.MAX_SAFE_INTEGER

  const startMin = timeToMin(startTime)
  const endMin   = timeToMin(endTime)
  // select('*') (not an explicit column list) so this can't 400 in the window
  // where this code is deployed but migration 033 hasn't been applied yet — a
  // missing room_requirement column just reads as undefined → 'any', i.e. the
  // pre-033 flat-count behaviour. A hard column list would 400 and make the
  // whole query return nothing, silently opening every room = overbooking.
  let bookingsQuery = admin.from('bookings').select('*').eq('date', date).in('status', ['pending', 'confirmed'])
  if (excludeBookingId) bookingsQuery = bookingsQuery.neq('id', excludeBookingId)
  const [capacity, typeCapacity, { data: bookings }] = await Promise.all([
    getRoomCapacity(admin, date),
    getRoomTypeCapacity(admin),
    bookingsQuery,
  ])
  ...
  // Bookings that need no room don't hold one.
  const inUse = overlapping.filter(b => (b.room_requirement ?? 'any') !== 'none').length
  const effectiveCapacity = maxConcurrent ? Math.min(capacity, maxConcurrent) : capacity
  let available = Math.max(0, effectiveCapacity - inUse)

  // Specialised scarcity: only bookings needing the SAME type compete for it.
  if (roomRequirement === 'wet' || roomRequirement === 'couples') {
    const typeCount = typeCapacity[roomRequirement]
    if (typeCount != null) {
      const typeInUse = overlapping.filter(b => b.room_requirement === roomRequirement).length
      available = Math.min(available, Math.max(0, typeCount - typeInUse))
    }
  }
  return available
}
...
export async function checkSlotCapacity(admin, { treatmentId, date, startTime, endTime, excludeBookingId }) {
  const [cfg, bufferMin, roomRequirement] = await Promise.all([
    getSlotConfig(admin, treatmentId),
    getTurnaroundMin(admin),
    getRoomRequirement(admin, treatmentId),
  ])
  const [{ data: closure }, therapistIds, rooms] = await Promise.all([
    ...
    getAvailableRoomCount(admin, { date, startTime, endTime, excludeBookingId, maxConcurrent: cfg.max_concurrent, bufferMin, roomRequirement }),
  ])
...
export function capacityErrorFromDb(error) {
  ...
  // migration 033: the one wet room / couple's room is already taken for this
  // window. Same guest-facing outcome as any other full slot.
  if (msg.includes('ROOM_TYPE_FULL_'))         return { reason: 'no_room' }
  return null
}
...
export async function getAvailableSlots(admin, { date, treatmentId, duration, addonMinutes = 0, excludeBookingId, asOf }) {
  ...
  const [qualified, required, roomRequirement] = await Promise.all([
    getQualifiedTherapistIds(admin, treatmentId),
    getRequiredTherapistCount(admin, treatmentId),
    getRoomRequirement(admin, treatmentId),
  ])
  ...
  const slots = await Promise.all(upcoming.map(async (time) => {
    ...
    const [freeTherapists, roomsAvailable] = await Promise.all([
      ...
      getAvailableRoomCount(admin, { date, startTime: time, endTime, excludeBookingId, maxConcurrent: cfg.max_concurrent, bufferMin: turnaroundMin, roomRequirement }),
    ])
...
export async function getBookableTreatmentsAt(admin, { date, startTime, durationPref }) {
  ...
  // NOTE: no early "any room free?" shortcut here. It used to `return []` when
  // no standard room was free — but reflexology needs NO room (chairs) and a
  // couple's/wet booking cares only about ITS own room type, so that shortcut
  // would wrongly hide bookable treatments. The authoritative per-treatment
  // room check (room type included) happens in checkSlotCapacity below.
