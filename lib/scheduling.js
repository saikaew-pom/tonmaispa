...
export async function getFreeTherapistIds(admin, { therapistIds, date, startTime, endTime, excludeBookingId, bufferMin = 0 }) {
  ...
  return therapistIds.filter(id => {
    ...
    const busy = (bookings ?? []).some(b => {
      ...
      return overlaps(startMin, endMin + bufferMin, bStart, bStart + b.duration + bufferMin)
    })
    return !busy
  })
}
...
export async function getAvailableSlots(admin, { date, treatmentId, duration, excludeBookingId, asOf }) {
  // Whole-spa closure
