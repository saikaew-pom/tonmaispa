...
async function callMiniMax(payload, locale) {
  ...
  try {
    const resp = await client.messages.create({
      model: MINIMAX_MODEL,
      max_tokens: 4000,
      system: `Translate the string values in this JSON object from English to ${LOCALE_NAMES[locale]}. This is copy for an upscale, calm garden spa in Phuket, Thailand — preserve that tone. Keep the proper noun "Ton Mai Spa" untranslated. Return ONLY a valid JSON object with the exact same keys, translated values, and no other text.`,
