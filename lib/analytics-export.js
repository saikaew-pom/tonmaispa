...
const styles = StyleSheet.create({
  ...
  interpBox: { backgroundColor: '#EFF3F1', borderRadius: 6, padding: 12, marginBottom: 8 },
  interpHeadline: { fontSize: 13, color: BRAND_GREEN, fontWeight: 700, marginBottom: 6 },
  interpBody: { fontSize: 10, color: '#4A4745', lineHeight: 1.4, marginBottom: 6 },
  interpSoWhatLabel: { fontSize: 8, color: BRAND_GOLD, textTransform: 'uppercase', fontWeight: 700, marginBottom: 2 },
  actionRow: { fontSize: 10, color: '#1C1917', marginBottom: 3 },
})
...
function InterpretationSection({ interpretation }) {
  ...
  return React.createElement(View, { style: styles.interpBox },
    React.createElement(Text, { style: styles.interpHeadline }, interpretation.headline),
    React.createElement(Text, { style: styles.interpBody }, interpretation.summary),
    React.createElement(Text, { style: styles.interpSoWhatLabel }, 'So what?'),
    React.createElement(Text, { style: styles.interpBody }, interpretation.soWhat),
    ...(interpretation.actions ?? []).map((a, i) =>
      React.createElement(Text, { key: i, style: styles.actionRow }, `${i + 1}. ${a}`)),
  )
}

function AnalyticsPdfDocument({ summary, interpretation }) {
  return React.createElement(Document, null,
    React.createElement(Page, { size: 'A4', style: styles.page },
      ...
      React.createElement(InterpretationSection, { interpretation }),
...
export async function renderAnalyticsPdf(summary, interpretation) {
  return renderToBuffer(React.createElement(AnalyticsPdfDocument, { summary, interpretation }))
}
