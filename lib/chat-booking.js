...
import { z } from 'zod'
import { sendEmail, bookingGuestHtml, bookingOwnerHtml, bookingRescheduledHtml } from '@/lib/brevo'
...
export async function confirmRescheduleDraft(admin, { sessionId, token, settings }) {
  ...
  let bookingQuery = admin.from('bookings')
    .select('id, ref_code, status, customer_id, chat_session_id, guest_name, guest_email').eq('id', draft.booking_id)
  ...
  if (error) return { ok: false, status: 500, error: 'Could not update the booking. Please contact the spa team.' }
