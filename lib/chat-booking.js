...
// Exported so the booking-engine test suite can assert the add-on tail
// survives the parse — z.object() strips undeclared keys silently, and that
// silence is exactly what let the occupancy drop out of the confirm step.
export const rescheduleDraftSchema = z.object({
  ...
  duration: z.number().int().min(30).max(480),
  // MUST be declared: z.object() STRIPS undeclared keys, so without this the
  // add-on tail prepareRescheduleDraft wrote into the draft would vanish at
  // parse time and confirmRescheduleDraft would re-check (and pick a
  // therapist for) only the BILLED window — under-reserving the real stay.
  addon_minutes: z.number().int().min(0).max(240).optional(),
...
export async function listSessionBookings(admin, sessionId) {
  ...
  let query = admin.from('bookings')
    .select('id, ref_code, date, time_slot, duration, addon_minutes, price, status, treatment_id, spa_treatments(name)')
...
export async function getRescheduleAvailability(admin, sessionId, rawInput) {
  ...
  const { slots, closed } = await getAvailableSlots(admin, {
    ...
    addonMinutes: booking.data.addon_minutes ?? 0,
...
export async function prepareRescheduleDraft(admin, sessionId, rawInput) {
  ...
  // Move the whole footprint, add-on tail included, or the JS pre-check would
  // pass a slot the capacity trigger then rejects (NEW.addon_minutes survives
  // the update) — a confusing 409 for the guest.
  const endTime = addMinutesStr(newTime, booking.data.duration + (booking.data.addon_minutes ?? 0))
  ...
  const draft = {
    ...
    duration: booking.data.duration,
    addon_minutes: booking.data.addon_minutes ?? 0,
    price: booking.data.price,
  }
...
export async function confirmRescheduleDraft(admin, { sessionId, token, settings }) {
  ...
  const capacity = await checkSlotCapacity(admin, {
    ...
    endTime: addMinutesStr(draft.new_time_slot, draft.duration + (draft.addon_minutes ?? 0)),
