...
const bookingDraftSchema = z.object({
  ...
  // Just a placeholder at this stage if the AI has it — the real phone
  // (with country code) and email are captured and validated on the
  // review card itself, right before confirm. Never required here.
...
// Final contact details are collected via a structured form in the review
// card, not trusted from whatever the AI parsed out of free-text chat —
// guest_phone here must already be a full E.164-style number (country code
// + number, composed client-side from an explicit country selector) and
// guest_email is required, closing the gap where an international guest
// might type only a local-format number with no country code.
...
export async function confirmBookingDraft(admin, { sessionId, token, settings, sendNotifications = true, guest_phone, guest_email }) {
  ...
  // before submitting — never trust a phone/email the AI may have parsed
  // loosely out of free-text chat. Required regardless of what the draft
  // already had, so an international guest can't slip through with a
  // country-code-less number.
  const contact = contactDetailsSchema.safeParse({ guest_phone, guest_email })
  if (!contact.success) {
    return { ok: false, status: 400, error: contact.error.issues[0]?.message ?? 'Please provide a valid phone number (with country code) and email.' }
  }
  ...
  const customerId = await upsertCustomer(admin, { name: draft.guest_name, phone: contact.data.guest_phone, email: contact.data.guest_email })
    ...
    await Promise.allSettled([
      ...
      ownerEmail && sendEmail({
        ...
        html: bookingOwnerHtml({
          name: draft.guest_name, phone: contact.data.guest_phone, refCode: booking.ref_code,
