...
// Fetch the current delivery status of a sent message straight from Twilio.
// "Send succeeded" only means Twilio ACCEPTED the message — WhatsApp can still
// silently drop it (e.g. error 63016: freeform message outside the guest's
// 24-hour session window). Callers poll this right after sending so staff see
// the real outcome instead of an optimistic "sent".
export async function fetchMessageStatus(sid) {
  const accountSid = requiredEnv('TWILIO_ACCOUNT_SID')
  const apiKeySid = process.env.TWILIO_API_KEY_SID?.trim() || accountSid
  const apiKeySecret = process.env.TWILIO_API_KEY_SECRET?.trim() || requiredEnv('TWILIO_AUTH_TOKEN')
  ...
  const result = await response.json()
  if (!response.ok) throw new Error(result.message || 'Could not fetch message status from Twilio')
  return {
    status: result.status,                    // queued | sent | delivered | failed | undelivered ...
    errorCode: result.error_code ?? null,     // e.g. 63016 = outside 24h window
